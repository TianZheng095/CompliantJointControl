...
P = cell(1,2);
P{1}=[0;0;0];P{2}=[l1;0;0];
% 每个连杆质心的位置矢量
Pc = cell(1,2);
Pc{1}=[0;0;0];Pc{2}=[l1;0;0];
% 连杆质量
m = cell(1,2);
m{2}=m1;
% 惯性张量
...
% 连杆间角速度和角加速度
w = cell(1,2);dw = cell(1,2);
w{1}=[0;0;0];dw{1}=[0;0;0];% 机器人底座不旋转
% 连杆坐标原点和质心加速度
dv = cell(1,2);dvc = cell(1,2);
dv{1}=[0;g;0];% 重力因素

% 关节速度和加速度
dq = cell(1,2); ddq = cell(1,2); 
dq{2}=[0;0;dq1];
ddq{2}=[0;0;ddq1];

% 末端执行器没有力
f = cell(1,3);n = cell(1,3);
f{3}=[0;0;0];
n{3}=[0;0;0];
